...
export default function TopMenu({
  ...
  const _className = classNames(
    className,
    "top-0 left-0 fixed w-full p-2 flex bg-[#252C39]"
  );

  return <nav className={_className}>{children}</nav>;
}
...
export function SubpageTopNavigation({
  className,
  title,
  children,
}: {
  ...
  return <TopMenu className={_className}>{pageTitle}</TopMenu>;
}
